export default function RubricContent() {
  return (
    <div className="text-stone-800">
      <h1 className="text-2xl lg:text-3xl font-black mb-8 text-stone-800 border-b-4 border-amber-300 pb-4 inline-block">
        📖 周杰伦歌曲评分细则
      </h1>
      ...
      <Section title="一" heading="核心维度与权重">
        ...
        <div className="bg-amber-50 border-l-4 border-amber-400 p-4 rounded-r-lg text-lg text-stone-700 font-bold mt-4 shadow-sm">
          总分公式：总分 = 旋律×0.3 + 编曲×0.25 + 歌词×0.2 + 演唱×0.15 + 创新×0.1<br/>
          <span className="text-base text-stone-500 font-normal">（每项0-100分，总分0-100分）</span>
        </div>
      </Section>
      ...
      <div className="mb-10">
        <h2 className="text-2xl font-bold text-stone-800 mb-6 flex items-center">
          <span className="bg-stone-800 text-white w-8 h-8 rounded-full flex items-center justify-center mr-3 text-lg shrink-0">二</span>
          各维度评分细则
        </h2>
        ...
        <DimensionCard
          ...
          extra={
            <div className="bg-emerald-50 p-3 rounded-lg mb-6 text-emerald-800 border border-emerald-200 text-base">
        ...
        <DimensionCard
          ...
          extra={
            <div className="bg-rose-50 p-3 rounded-lg mb-6 text-rose-800 border border-rose-200 text-base">
      ...
      <Section title="三" heading="打分操作流程">
        <div className="bg-white p-6 lg:p-8 rounded-2xl shadow-sm border border-stone-200 text-base text-stone-700 leading-loose">
          ...
          <ol className="list-decimal pl-6 space-y-4">
            ...
            <li><strong>凭直觉定档</strong>（S/A/B/C/D），五个维度分别判断：
              <ul className="list-[circle] pl-6 mt-2 text-stone-500">
                <li>这个旋律是S档还是A档？</li>
                <li>这个编曲是S档还是A档？</li>
                <li>…如此类推</li>
              </ul>
            ...
            <li><strong>在档位分值区间内给具体分</strong>（取整数即可，如88、92）：
              <ul className="list-[circle] pl-6 mt-2 text-stone-500 grid grid-cols-2 gap-x-4">
      ...
      <Section title="四" heading="评分校准系统">
        <div className="bg-white p-6 lg:p-8 rounded-2xl shadow-sm border border-stone-200">
          <p className="text-base font-bold text-stone-700 mb-4">先给以下锚点歌强制打分，作为你的评分标尺：</p>
          <table className="rubric-table mb-6">
            <thead>
              <tr><th>刻度等级</th><th>代表歌曲</th><th>参考总分区间</th></tr>
            </thead>
            <tbody>
              <tr><td className="font-black text-purple-600">S 神作</td><td className="text-base">《以父之名》《夜的第七章》《青花瓷》</td><td className="font-bold">95-100</td></tr>
              <tr><td className="font-black text-red-500">A+ 准神</td><td className="text-base">《夜曲》《七里香》《双截棍》</td><td className="font-bold">90-94</td></tr>
              <tr><td className="font-black text-red-500">A 优质</td><td className="text-base">《爱在西元前》《晴天》《一路向北》</td><td className="font-bold">85-89</td></tr>
              <tr><td className="font-black text-amber-500">B 良好</td><td className="text-base">《浪漫手机》《听妈妈的话》</td><td className="font-bold">75-84</td></tr>
              <tr><td className="font-black text-green-500">C 及格</td><td className="text-base">《英雄》（游戏曲）</td><td className="font-bold">60-74</td></tr>
            </tbody>
          </table>
          <div className="bg-stone-50 p-4 rounded-lg border border-stone-200 text-base text-stone-700">
      ...
      <div className="text-center text-stone-400 text-sm mt-12 border-t border-stone-200 pt-8">
  ...
  return (
    <div className="mb-10">
      <h2 className="text-2xl font-bold text-stone-800 mb-4 flex items-center">
        <span className="bg-stone-800 text-white w-8 h-8 rounded-full flex items-center justify-center mr-3 text-lg shrink-0">{title}</span>
        {heading}
      </h2>
      {children}
    </div>
...
function DimensionCard({ title, colorClass, borderClass, bgClass, points, anchors, refSongs, extra }: {
  title: string;
  colorClass: string;
  borderClass: string;
  bgClass: string;
  ...
  const tierColorMap: Record<string, string> = {
    S: 'text-purple-600', A: 'text-red-500', B: 'text-amber-500', C: 'text-green-500', D: 'text-stone-500',
  };

  return (
    <div className={`rounded-2xl p-6 lg:p-8 mb-8 shadow-sm border border-stone-200 ${bgClass}`}>
      <h3 className={`text-xl lg:text-2xl font-black ${colorClass} mb-6 border-b ${borderClass} pb-3`}>{title}</h3>

      <h4 className="text-base font-bold text-stone-700 mb-3 bg-stone-100 inline-block px-3 py-1 rounded">考察要点：</h4>
      <ul className="list-disc pl-6 text-stone-600 space-y-2 mb-6 text-base">
        {points.map((p, i) => <li key={i}>{p}</li>)}
      </ul>

      {extra}

      <h4 className="text-base font-bold text-stone-700 mb-3 bg-stone-100 inline-block px-3 py-1 rounded">定档锚点：</h4>
      <table className="rubric-table mb-6">
        <thead>
      ...
      <div className="bg-white/70 p-4 rounded-lg border border-stone-200">
        <h4 className="text-sm font-bold text-stone-700 mb-2">参考锚点歌：</h4>
        <ul className="text-stone-600 space-y-1 text-base">
